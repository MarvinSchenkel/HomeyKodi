                                ...
                                if (resultGetItem.item.type === 'movie' || resultGetItem.item.type === 'movies') {
                                    ...
                                    this._kodiConnection.run('VideoLibrary.GetMovieDetails', movieParams)
                                        .then((movieResult) => {
                                            ...
                                            artUrl = (movieResult.moviedetails.art.poster) ? decodeURIComponent(movieResult.moviedetails.art.poster).replace('image://', '').replace('.jpg/', '.jpg').replace('http://', 'https://') : ''
                                            console.log(artUrl)
                                        })
                                        .catch(() => {
                                        ...
                                        .then(() => {
                                            if (movieTitle !== '') {
                                                let movieObj = new Movie(((resultGetItem.item.id) ? resultGetItem.item.id : -1), movieTitle, artUrl)
                                    ...
                                    let episodeParams = {
                                        ...
                                        properties: ['showtitle', 'season', 'episode', 'title', 'art']
                                    }
                                    this._kodiConnection.run('VideoLibrary.GetEpisodeDetails', episodeParams)
                                        .then((episodeResult) => {
                                            ...
                                            artUrl = (episodeResult.episodedetails.art['season.poster']) ? decodeURIComponent(episodeResult.episodedetails.art['season.poster']).replace('image://', '').replace('.jpg/', '.jpg').replace('http://', 'https://') : ''
                                        })
                                        .catch((err) => {
